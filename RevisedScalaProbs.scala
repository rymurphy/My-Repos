...
object Exercises {
  ...
  def length[A](as: List[A]): Int =
    List.foldLeft(as, 0, (x: Int, y: A) => add(x, 1))
  ...
  def filter[A](as: List[A], f: A => Boolean): List[A] = error("todo")
    List.foldLeft(as, Empty, (head: A, tail: List[A]) => if (f(head) == true) Cons(head, tail) else tail)
  ...
  def append[A](x: List[A], y: List[A]): List[A] =
    List.foldRight(x, y,(head: A, tail: List[A]) => Cons(head, tail))
  ...
  def flatten[A](as: List[List[A]]): List[A] =
    List.foldLeft(as, Empty,(headList:List[A], tailList: List[A]) => append(tailList, headList))
